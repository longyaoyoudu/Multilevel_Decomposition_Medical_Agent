    ...
    try:
        ...
        for entity_name, types in entity_dict.items():
            for entity_type in types:
                ...
                rels = chatbot.link_entity_rel(query, entity_name, entity_type, context=context)
        ...
        # Step 3: 构建Prompt - 包含历史上下文
        logs.append(color_log("【步骤3】构建提示词...", "system"))
        fact_prompt = chatbot.format_prompt(query, facts_list, context=context)
...
with gr.Blocks(
    ...
    def process_query(query, history):
        ...
        try:
            ...
            if not entity_dict:
                logs.append(color_log("⚠️ 未识别到相关实体", "warning"))
                chat_html = update_chat_display(history + [(query, "抱歉，这个问题超出了我的知识范围。")])
                return "抱歉，这个问题超出了我的知识范围。", "\n".join(logs), history + [
                    (query, "抱歉，这个问题超出了我的知识范围。")], chat_html
            ...
            # Step 3: 构建Prompt
            logs.append(color_log("【步骤3】构建提示词...", "system"))
            fact_prompt = chatbot.format_prompt(query, facts_list)  # 移除了context参数
            logs.append(color_log(f"提示词内容:\n{fact_prompt}", "info"))
    ...
    def update_chat_display(history):
        ...
        for user_msg, assistant_msg in history:
            ...
            <div class="user-message">
                <strong>您:</strong> {user_msg}
            </div>
            <div class="assistant-message">
                <strong>助手:</strong> {assistant_msg}
            </div>
            """
        chat_html += "</div>"
        return chat_html
    ...
    def clear_chat():
        """清除所有对话历史"""
        return [], "<div class='log-panel'>系统准备就绪，日志将显示在这里...</div>", "<div class='chat-history'><div style='text-align: center; padding: 20px; color: var(--text-secondary);'>对话历史已清除</div></div>"
    ...
    example_input.change(
        ...
        inputs=[example_input],
        outputs=[input_query, chat_history]
    )
    ...
    clear_btn.click(
        fn=clear_chat,
        inputs=[],
        outputs=[chat_history, process_logs, chat_display]
    )

    # 注入JavaScript代码 - 处理示例点击并填充输入框
    js_code = """
    function() {
        // 为所有示例项添加点击事件
